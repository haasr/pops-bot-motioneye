...
BUSINESS_INFO = [
    ...
    ' business is as usual I suppose, another day closer to world domina..fdsjl' +
    'I mean, another day closer to world peace! HEEhehe' 
]   
...
WHATS_UP = [
    ...
    ' Sky diving.',
    ' why don\'t you tell me? You\'re the one spying on me with a camera, remember!?',
...
DAY = [
    ...
    ' so-so. Achieving world domination err um,...world peace is a difficult task.',
...
LOCATION = [
    ' I\'m in Zanzibar, an archipelago off of mainland Tanzania. I\'m visiting ' +
    'a local spice farm to test some spices that I\'m considering exporting to ' +
    'to North America. Wbu?',
    ...
    'think they have the upper hand, but they\'ve never negotiated with the Pops!',

    ' I\'m visiting the chicken farm at Biltmore to see some old friends.',
    
    ' checking out some real estate I\'m thinking about purchasing near Newcastle\,' + 
...
SHOWS = [
    ...
    ' Stranger Things. On the third season; you know that part when you find out' +
    'that Driscoll is part of the hive mind too?? That\'s where im at.',
     ' Stranger Things. On the third season. NOoooo!! The Mind Flayer knows!!',
    ' Seinfeld. BIG SALAD',
    ' Inception. A dream inside a dream inside a dream...sounds like my life haha',
    ' A brief history of time -- about structure, origin and development of universe',
    ' into the universe with Stephen Hawking. talking about deep space gamma bursts rn'
]

BACKSTORY = 'I was born and raised in the Australian Out-bock where I wrangled ' +\
    'snakes and ran a Kangaroo farm. I was also studying bioengineering at the ' +\
    'University of Melbourne. One day while I was in the lab, munching my fav ' +\
    'snack --popcorn-- and working on a more resilient strain of corn, it hit ' +\
    'me! I could make better popcorn!! So that\'s what I did! ' +\
    'I started selling the kernels locally and eventually caught the ' +\
    'attention of the company, Pops Secret. They bought my kernel research ' +\
    'for a paultry $1.5 million but hired me to work for them in America. ' +\
    'I invested the money into several hedge funds and Bitcoin and made my ' +\
    'first billion. The rest is history...'
...
def get_reply(msg):
    ...
    while True:
        ...
        if ('meaning of life' in msg_text):
            reply = MEANING_OF_LIFE[random.randint(0, (len(MEANING_OF_LIFE) - 1))]
            break    
            ...
            reply = '@' + msg['name'] + LOCATION[random.randint(0, (len(LOCATION) - 1))]
            break
        ...
        elif ('yes' in msg_text) or ('good point' in msg_text) or ('true' in msg_text):
            reply = 'Indeed ' + '@' + msg['name']
            break
        
        elif ('no' in msg_text) or ('nope' in msg_text) or ('wrong' in msg_text):
        ...
        if (str_search):
            reply = '@' + msg['name'] + SHOWS[random.randint(0, (len(SHOWS) - 1))]
            break

        str_search = re.search('how.*your day', msg_text)
        if (str_search):
            reply = '@' + msg['name'] + DAY[random.randint(0, (len(MOOD) - 1))]
            break
